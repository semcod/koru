...
Extracted from :mod:`koru.autonomy.cycle.cycle_chat_activity` (R7a — FAZA 2)
...
def _create_llm_needs_input_operator_task(
    *,
    ...
    try:
        # Resolve ``create_nl_task`` via ``koru.autonomy.cycle.cycle`` so existing
        # tests that monkeypatch ``koru.autonomy.cycle.cycle.create_nl_task`` keep
