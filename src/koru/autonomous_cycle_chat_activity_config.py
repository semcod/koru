"""
Backward compatibility shim for koru.autonomy.cycle.cycle_chat_activity_config module migration.

This module maintains backward compatibility by re-exporting from the new
autonomy.cycle.cycle_chat_activity_config submodule. Remove this shim after one release.
"""

# Re-export everything from the new module location
from koru.autonomy.cycle.cycle_chat_activity_config import *  # noqa: F401, F403

# Also expose the module for test monkeypatching and private function access
from koru.autonomy.cycle import cycle_chat_activity_config as _module_impl  # noqa: F401
import sys
_current_module = sys.modules[__name__]
for attr in dir(_module_impl):
    if not attr.startswith("__"):
        if not hasattr(_current_module, attr):
            setattr(_current_module, attr, getattr(_module_impl, attr))
