...
def render_planfile_refactor_handoff(project: Path) -> str:
    ...
    return (
        ...
        f"{prompt_hint}"
        "- W IDE: **dołącz** (`@`) powyższy plik analizy do czatu albo wklej fragmenty REFACTOR / HEALTH.\n"
        "- Dla każdej sensownej jednostki pracy utwórz **osobny** ticket planfile (queue `default` lub wg projektu), "
        "z etykietą `llm-ready`, krótkim tytułem i opisem z odwołaniem do ścieżek plików z analizy.\n"
        "- Po utworzeniu: `task tickets:next` / `planfile` — tak, by kolejka `koru autonomous` miała co wykonać.\n"
