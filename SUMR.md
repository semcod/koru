...
- **generated_from**: pyproject.toml, Taskfile.yml, testql(2), app.doql.less, goal.yaml, .env.example, project/(1 analysis files)
...
tasks:
  ...
  install:tools:
    desc: Install all underlying LLM tools (planfile, regix, redup, vallm, sumd, ...)
    cmds:
      - '[ -d ../planfile ] && pip install -e ../planfile || pip install planfile'
      - pip install regix redup vallm prefact pfix sumd code2llm doql
      - 'echo "✓ Core tools installed. For LLM-backed tools (redsl, llx, aider) install separately."'
