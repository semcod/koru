...
### Fixed
- ``koru ide doctor`` skips ``trustedPublishers.missing`` when classic Cursor
  exthost already activated the Koru VSIX (state.vscdb may lack ``semcod`` while
  the extension still loads from ``--extensions-dir``).
...
### Fixed
- ``koru autopilot drive/status`` no longer treats Taskfile ``KORU_AUTOPILOT_INSTANCE=auto``
  as the bare ``koru-autopilot.sock`` lane when ``--ide`` selects a concrete IDE
  (parity with ``koru ide doctor``).

## 0.1.455

- fix(doctor): sync koru-autopilot-vscode package-lock root version to 0.2.13 (clears lock_version_mismatch WARN)
...
### Fixed
- ``koru ide doctor --fix`` no longer treats Taskfile ``KORU_AUTOPILOT_INSTANCE=auto``
  as the bare ``koru-autopilot.sock`` lane when a concrete IDE is selected.
...
### Other
- Update packages/koruide/src/koruide/plugin_version.py
...
### Test
- Update tests/test_agent_availability.py
...
- Update tests/test_cycle_orchestrator.py
...
### Test
- Update tests/test_nxdo_discovery.py
...
- Update plugins/koru-autopilot-vscode/package.json
- Update plugins/koru-autopilot-vscode/src/ides/vscode.ts
...
- Update scripts/write-ide-plugin-tests.py
- ... and 2 more files
...
- Update tests/test_autopilot_daemon.py
- Update tests/test_doctor.py
- Update tests/test_drive_repair_policy.py
...
- Update .vdisplay/2026-06-10T14-32-42Z__local__cli/steps/0001/command.dsl.txt
- Update .vdisplay/2026-06-10T14-32-42Z__local__cli/steps/0001/diagnostics.json
- ... and 15 more files
