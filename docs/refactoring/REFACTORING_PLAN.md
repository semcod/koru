...
- [~] **R5b**: pozostała część `autopilot/cli_command.py` – podział na `commands/{drive,status,shutdown,doctor,handoff,trace,install_unit,manage,...}.py` (częściowo: wydzielono `commands/drive.py` 2 funkcje + `commands/status.py` 2 funkcje + `commands/shutdown.py` 1 funkcja; cli_command.py: 421 -> 333 LOC, **-88 LOC, -20.9%**; +25 testów; pozostałe: calibrate, session-start, ide-list, doctor, setup-host, manage, install-plugin, install-plugin-jetbrains, handoff, tail, install-unit)
