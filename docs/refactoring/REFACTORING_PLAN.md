...
- [~] **R5b**: pozostała część `autopilot/cli_command.py` – podział na `commands/{drive,status,shutdown,handoff,doctor,trace,install_unit,manage,...}.py` (częściowo: wydzielono `commands/drive.py` 2 funkcje + `commands/status.py` 2 funkcje + `commands/shutdown.py` 1 funkcja + `commands/handoff.py` 2 funkcje; cli_command.py: 421 -> 292 LOC, **-129 LOC, -30.6%**; +35 testów; pozostałe: calibrate, session-start, ide-list, doctor, setup-host, manage, install-plugin, install-plugin-jetbrains, tail, install-unit)
