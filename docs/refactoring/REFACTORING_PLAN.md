...
- [~] **R6**: `koruide/daemon/handlers.py` – podział per typ wiadomości (częściowo: wydzielono `handlers_drive.py` 7 funkcji + `handlers_hello.py` 6 funkcji + `handlers_ack.py` 9 funkcji; handlers.py: 1295 → 420 LOC, **-875 LOC, -67.6%**; +34 testów; pozostałe: plugin_event, status, shutdown, ping, console_log)
