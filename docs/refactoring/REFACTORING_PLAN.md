...
- [~] **R6**: `koruide/daemon/handlers.py` – podział per typ wiadomości (częściowo: wydzielono `handlers_drive.py` z 7 funkcji obsługi drive: `handle_drive`, `_drive_via_plugin`, `_try_os_injector_drive`, `_drive_via_keyboard`, `_resolve_keyboard_drive_selection`, `_drive_via_os_injector_backend`, `_drive_via_keyboard_backend`; handlers.py: 1295 → 886 LOC, **-409 LOC, -31.6%**; +14 testów; pozostałe handlery: hello, status, ack, plugin_event)
