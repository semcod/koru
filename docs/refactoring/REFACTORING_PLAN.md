...
- [~] **R5b**: pozostała część `autopilot/cli_command.py` – podział na `commands/{drive,status,doctor,handoff,trace,install_unit,manage,...}.py` (częściowo: wydzielono `commands/drive.py` 2 funkcje + `commands/status.py` 2 funkcje; cli_command.py: 421 -> 327 LOC, **-94 LOC, -22.3%**; +21 testów; pozostałe: calibrate, session-start, shutdown, ide-list, doctor, setup-host, manage, install-plugin, install-plugin-jetbrains, handoff, tail, install-unit)
