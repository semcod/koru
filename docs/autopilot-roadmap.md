...
|-----------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|-------------------------------------------------------------|--------|
| **R1**    | `_HANDLERS` is a module-level dict referencing private `_handle_*` methods through thin `_h_*` wrappers. Replace with bound methods set in `__init__`, or a `@_handler("type")` decorator collected on the class. Cuts ~30 lines and removes the indirection.                                                                                                                                                                                                                                  | `src/koru/autopilot/daemon.py:_HANDLERS`                    | S      |
| **R2**    | The integration tests duplicate "start daemon in a thread, connect, read frames" plumbing. Extract a `pytest` fixture (`@pytest.fixture` returning `(daemon, sock_path, line_reader)`) plus a small helper context manager. Will drop ~80 lines and make adding new socket-level tests trivial.                                                                                                                                                                                                | `tests/test_autopilot_daemon.py`                            | S      |
...
| **R8**    | The VS Code extension uses clipboard + paste + submit. That clobbers the user's clipboard if our `await readText()/writeText(previous)` round-trip races with a paste in another app. Use a private hidden text area or the proposed `vscode.chat.sendMessage` API once it stabilises.                                                                                                                                                                                                          | `plugins/koru-autopilot-vscode/src/extension.ts:injectChat` | M      |
| **R9**    | `daemon._handle_drive` mixes plugin-path and keyboard-sim branches in one method (CC ≈ 9). Extract `_drive_via_plugin` and `_drive_via_keyboard` so each is testable in isolation. Aligns with the pyqual `cc_max=20` gate.                                                                                                                                                                                                                                                                       | `src/koru/autopilot/daemon.py`                              | S      |
...
1. **R1, R2** (mechanical cleanups) before Phase 2 — they make every subsequent test cheaper to write.
2. **R12** (schema cap) before Phase 3 — protocol freezes once JetBrains ships.
3. **R8** (clipboard race) is the only refactor that has a user-visible
   bug today; bump it if anyone complains about clipboard contents
   disappearing.
4. Everything else is opportunistic — pick up an item whenever you
