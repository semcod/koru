...
> **Audience:** LLM agent (Windsurf, Cursor, Claude Code, TILLM shell clients). Read this
...
> invoke remote LLM APIs (`redsl improve`, `llx fix`, shell clients via `tillm`) — koru repos
...
| `llx fix` | Jak wyżej. |
| shell clients via `tillm` | Jak wyżej. |
