...
def apply_nl_with_diag(prompt: str, *, image: str | None = None, window: str | None = None, execute: bool = True, dry_run: bool = False, with_diagnostics: bool = False) -> dict[str, Any]:  # noqa: E501
    ...
    if diagnose_capture is not None:
        ...
        if diag and not diag.get("ok") and diag.get("verdict") == "stale_capture":
            if os.environ.get("KORU_IMGL_STALE_BLOCK", "") in {"1", "true", "yes"}:
                return {"ok": False, "blocked_by": "stale_capture", "diagnostics": {**diag, "verdict": "stale_capture_error"}}  # noqa: E501
        if diag and diag.get("verdict") == "blank_capture":
            if os.environ.get("KORU_IMGL_DIAG_BLOCK", "") in {"1", "true", "yes"}:
                return {"ok": False, "blocked_by": "capture_diagnose", "diagnostics": {**diag, "verdict": "blank_capture_error"}}  # noqa: E501

    return {"ok": True, "verb": "TYPE", "output": prompt, "data": {"execute": {"ok": True, "method": "xdotool", "dry_run": dry_run}}}  # noqa: E501
