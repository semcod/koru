...
class TestServe(unittest.TestCase):
    ...
    def test_dashboard_html_served_on_root(self) -> None:
        ...
        self.assertIn("ticketsTable", body)
        self.assertIn("open_tickets", body)
    ...
    def test_context_endpoint_lists_project_tickets(self) -> None:
        tickets = [
            {
                "id": "PLF-001",
                "name": "Queued ticket",
        ...
        with mock.patch(
            "koruapi.dashboard_routes.dashboard_context_payload",
        ) as context_payload_mock:
            context_payload_mock.return_value = {
                "project": str(self.project.resolve()),
                "policy": {},
                "environment": {},
                "ticket": {"id": "PLF-002"},
                "open_tickets": tickets[:1],
                "all_tickets": tickets,
            }
            status, ctype, body = _get(
                self.port,
                f"/api/context?project={self.project.resolve()}",
            )
            self.assertEqual(status, 200)
            self.assertIn("application/json", ctype)
            payload = json.loads(body)
            self.assertEqual(payload["ticket"]["id"], "PLF-002")
            self.assertEqual(
                [row["id"] for row in payload["all_tickets"]],
                ["PLF-001", "PLF-002"],
            )
            context_payload_mock.assert_called_once_with(self.project.resolve(), None)
    ...
    def test_api_config_get_and_post_persist_dashboard_settings(self) -> None:
        ...
        self.assertEqual(payload["dotenv"]["path"], str(self.project / ".env"))
        self.assertIn("auto", payload["ide_choices"])
