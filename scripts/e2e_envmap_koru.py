...
def _check_dependencies(desktop_uri, env2llm_registry) -> bool:
...
def _refresh_registry(env2llm_registry) -> str | None:
    ...
    return refresh.get("path")


def _print_uri_index(env2llm_registry) -> None:
...
def _resolve_prompts(desktop_uri, prompts: list[str], *, doql_path: str | None = None) -> None:
...
def _desktop_only_baseline(desktop_uri) -> None:
...
def main() -> int:
    ...
    if not _check_dependencies(desktop_uri, env2llm_registry):
        return 1

    registry_path = _refresh_registry(env2llm_registry)
    _print_uri_index(env2llm_registry)
    ...
    _resolve_prompts(desktop_uri, prompts)

    _section("4. NL resolve — WITH env2llm registry (command:// aware)")
    if registry_path:
        _resolve_prompts(desktop_uri, prompts, doql_path=registry_path)
    else:
        print("  (skipped — no registry)")

    _desktop_only_baseline(desktop_uri)
    _mcp_tool_smoke()
    _print_summary()
    return 0
