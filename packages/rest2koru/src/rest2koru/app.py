"""FastAPI REST adapter for dsl2koru."""
...
from dsl2koru.schema_registry import schema_for_verb, validate_schemas
from fastapi import FastAPI, Request
from fastapi.responses import JSONResponse, Response


def create_app() -> FastAPI:
    app = FastAPI(title="rest2koru", version="0.1.0")
    ...
    def get_events(project: str = ".") -> JSONResponse:
        store = EventStore.for_project(Path(project))
        events = [event.to_dict() for event in store.read_all()]
        return JSONResponse(events)

    @app.get("/v1/proto")
    def get_proto() -> Response:
        proto_dir = Path(__file__).resolve().parents[3] / "dsl2koru" / "proto" / "dsl2koru" / "v1"
        parts = []
